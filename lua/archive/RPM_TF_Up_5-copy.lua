...
-- минутные ТФ: расчёт только во второй половине истории (в 2Ч меньше нагрузка)
local function minuteHalfHistoryStart()
    return floor(Size() / 2) + 1
end

local function minuteLayerActive(index, tf)
    if not isMinuteTf(tf) then
        return true
    end
    return index >= minuteHalfHistoryStart()
end
...
-- --- дивергенции RPM ---
-- kind (направление сигнала):
--   bear / bull          — классическая (разворот)
--   hidden_bear / hidden_bull — скрытая (продолжение)
--   extended_bear / extended_bull — расширенная (двойная вершина/дно)
--   weak_bear / weak_bull — тип III: цена активно, RPM почти flat
-- strength (сила расхождения, учебник I/II/III):
--   I   — сильная (классическая bear/bull)
--   II  — средняя (extended_*)
--   III — слабая (weak_*)
...
local DIV_EXTENDED_PCT      = 0.003   -- ~0.3% — «один уровень» для типа II
local DIV_WEAK_RPM_PCT      = 0.015   -- ~1.5% — RPM «почти не двигается»
local DIV_WEAK_PRICE_PCT    = 0.005   -- ~0.5% — цена «активно» меняет экстремум

-- номера линий в return OnCalculate (порядок как в Init)
local LINE = {
...
local function rpmRangeStats(layer, idx1, idx2)
    ...
    stats.toIdx = toIdx
    local rpmByIndex = layer.rpmByIndex
    if rpmByIndex == nil then
        return stats
    end
    for bar = fromIdx, toIdx do
        local v = rpmByIndex[bar]
        if v ~= nil then
...
local function Algo(Fsettings, ds)
    ...
    return function (index)
        ...
        local status, res = pcall(function()
            ...
            local halfStart = minuteHalfHistoryStart()

            if needMiddle then
                if isMinuteTf(middle) and index == halfStart then
                    clearArray(middlePrice)
                    fEmaMiddle = nil
                    fEmaMiddleSlow = nil
                    resetSegLayer(divMiddleLayer)
                end
                if minuteLayerActive(index, middle) then
            ...
            if needSmall then
                if isMinuteTf(small) and index == halfStart then
                    clearArray(smallPrice)
                    fEmaSmall = nil
                    fEmaSmallSlow = nil
                    resetSegLayer(divSmallLayer)
                end
                if minuteLayerActive(index, small) then
            ...
            if needUp then
                if isMinuteTf(up) and index == halfStart then
                    clearArray(upPrice)
                    fEmaUp = nil
                    fEmaUpSlow = nil
                    resetSegLayer(divUpLayer)
                end
                if minuteLayerActive(index, up) then
                    if (#upPrice == 0) then
            ...
            if needMiddle and minuteLayerActive(index, middle) and fEmaMiddle ~= nil and #middlePrice > 12 then
            ...
            if needSmall and minuteLayerActive(index, small) and fEmaSmall ~= nil and #smallPrice > 12 then
            ...
            if needUp and minuteLayerActive(index, up) and fEmaUp ~= nil and #upPrice > 12 then
                ...
                if divLogEnabled then
                    ...
                    if needSmall then
                        divLog("Small TF=", small, "div=", smallDivDraw, "label=", smallLabelDraw,
                            "segs=", #smallPrice, "rpmSamples=", divSmallLayer.rpmByIndex and "yes" or "no")
                    end
                    if needMiddle then
