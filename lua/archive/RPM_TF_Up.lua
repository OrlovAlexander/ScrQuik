...
local logFile = nil
-- logFile = io.open(_G.getWorkingFolder().."\\LuaIndicators\\RPM_TF_Up.txt", "w")
...
local function setFirstTime(tf, tfds, index, ds)
    ...
    local status, res = pcall(function()
        ...
        if (tf == "W1" and crT.week_day == 2) then
            setFirstBar = true;
        end
        if setFirstBar then
...
local function setPrice(tf, tfds, index, ds)
    local status, res = pcall(function()
        ...
        -- w1
        if (tf == "W1" and upT.week_day == 2 and newDay) then
        ...
        -- h1
        if (tf == "H1") then
            upTfSec = upTfSec + 1 * 60 * 60 - 1; -- в секундах
            if (upTfSec < crTfSec) then
                addNewBarInTfds = true;
            end
        end
        if (tf == "H2" and (newDay == true or tt.hour % 2 == 0)) then
            upTfSec = upTfSec + 1 * 60 * 60; -- в секундах
            if (upTfSec < crTfSec) then
                addNewBarInTfds = true;
            end
        end
